...
import { ArrowLeft, Phone, Mail, CreditCard, FilePlus, Trash2 } from "lucide-react"
...
export default function ClienteDetailPage() {
  ...
  const handleDelete = async () => {
    if (!window.confirm(`¿Eliminar a ${cliente?.nombre}? Se eliminarán también todas sus cotizaciones.`)) return
    setDeleting(true)
    try {
          ...
          <CardContent className="space-y-2 text-sm px-4 pb-4">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-emerald-400 font-medium">
                {cliente.cotizaciones.filter((c: CotizacionRow) => c.estado === "COMPLETADA").length}
              </span>
            </div>
          </CardContent>
        </Card>
